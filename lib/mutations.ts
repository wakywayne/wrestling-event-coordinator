...
const createEvent = async (
  event: Omit<partialEvent, "_id" | "eventApplicants">
  ...
  const cleanEventWithoutEventApplicants = cleanUndefinedOrNullKeys<
    Omit<Events, "_id" | "eventApplicants">
  >(dirtyEventWithoutEventApplicants);
  ...
  const insertEvent = async () => {
    ...
    const newEvent = await db.collection("events").insertOne(cleanEvent);
  ...
  const insertUserCreatedEvent = async (
    event: Partial<createdEvents>,
    eventId: ObjectId
  ) => {
    const client = await clientPromise;
    const db = client.db();
  ...
  const returnedEventId = await insertEvent().then(
    (returnedId: ObjectId): ObjectId => {
      ...
      const cleanEventWithoutId =
        cleanUndefinedOrNullKeys<Omit<Events, "_id">>(eventWithoutId);
      ...
      if (Array.isArray(eventWeights) && eventWeights.length) {
        ...
        const newEventWeights = eventWeights.map((weight) => {
          const numberWeight = Number(weight.weight);
        ...
        insertUserCreatedEvent(
          { ...cleanEventWithoutId, createdEventWeights: newEventWeights },
          returnedId
        );
      } else {
...
const updateEvent = async (
  createdBy: ObjectId,
  event: Omit<partialEvent, "createdBy">
): Promise<Events | undefined> => {
  ...
  try {
    ...
    const updateUserCreatedEvent = async (
      event: createdEvents,
      userId: ObjectId
    ) => {
      const client = await clientPromise;
      ...
      const newEvent = await db.collection("users").updateOne(
        {
          _id: new ObjectId(userId),
          "createdEvents.createdEventId": new ObjectId(_id),
        },
        { $set: { "createdEvents.$": eventWithConfirmedProperId } }
      );
      // return the created event's id
    ...
    // we redue the process we did earlier
    const cleanEventWithId =
      cleanUndefinedOrNullKeys<createdEvents>(eventWithId);
    ...
    if (Array.isArray(eventWeights) && eventWeights.length) {
      ...
      updateUserCreatedEvent(
        { ...cleanEventWithId, createdEventWeights: newEventWeights },
        createdBy
      );
    } else {
      const newEventWeights: weightsForUserCreatedEvents[] = [];
    ...
    const queries = await Promise.all([
      db
        ...
        .updateOne(
          ...
          { $push: { eventApplicants: { userId, name, weight } } }
        ),
      db.collection("users").updateOne(
        { _id: new ObjectId(userId) },
        {
...
const acceptOrRemoveApplicant = async (
  eventId: ObjectId,
  createdBy: ObjectId,
  ...
  if (boolean == true) {
    try {
      ...
      const query = await db
        .collection("events")
        .updateOne({ _id: new ObjectId(eventId), "weights.weight": weight }, [
          {
            $set: {
              newItem: {
                $reduce: {
  ...
  try {
    ...
    const query = await db
      ...
      .updateOne(
        ...
        { $set: { name, email, availableWeights } }
      );

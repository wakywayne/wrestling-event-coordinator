  ...
  function createEventClick(formValues: EventType) {
    ...
    if (cleanedFormValues.weights) {
      cleanedFormValues.weights.forEach((weight, index) => {
        ...
        const trueIfExists = representative.some((rep) => rep === weight)
          ? true
          : pushToRepAndHolder(weight);
        if (trueIfExists) {
          // @ts-ignore
    ...
    const arrayFideObject = Object.entries(holder).map(([key, value]) => ({
      weight: Number(key),
    ...
    <div className="relative myContainer">
      ...
      <div className="flex justify-center">
        <form
          ...
          onSubmit={handleSubmit(createEventClick)}
        >
          ...
          <div className="flex items-center justify-center w-full ">
            ...
            <input
              type="text"
              placeholder="103,113,120,126,132"
              {...register("weights", {
                setValueAs: (v: string) =>
                  v == ""
                    ? undefined
            ...
            />
            <AiFillInfoCircle
              className="inline ml-1 mr-12 cursor-pointer"
          ...
          {popUp && (
            <div className="z-10 p-2 bg-white border border-black rounded">
              <div className="flex justify-between">
                <span>Enter weights separated by commas</span>{" "}
                <BsX color="red" onClick={() => setPopUp(false)} />
              </div>
              <span className="block text-sm text-ellipsis">
                to add multiple spots at the same weight just repeat the weight
                in the list
              </span>
            </div>
          )}
